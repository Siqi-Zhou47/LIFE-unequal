...
class FEMBodyClass {
	...
	// Default constructor and destructor
public:
	FEMBodyClass() {iPtr = NULL; L0 = 0.0; angle0 = 0.0; bodyDOFs = 0; bcDOFs = 0; itNR = 0; resNR = 0.0; tCrit = 0.0; subRes = 0.0; subNum = 0.0; subDen = 0.0; qUniform = 0.0;};
	~FEMBodyClass() {};

	// Custom constructor for building corotational FEM body
	FEMBodyClass(IBMBodyClass *iBodyPtr, const array<double, dims> &pos, const array<double, dims> &geom, double angle, string nElementsStr, string BC, double rho, double E, double qUniform);

	void setUniformLoad(double q) { qUniform = q; }
...
private:
	...
	void constructRVector();
	void constructRVector_scaled(double factor);		// Get load vector
