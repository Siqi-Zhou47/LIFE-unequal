...
public:
	IBMBodyClass() {oPtr = NULL; ID = 0; flex = eRigid; bodyType = eCircle; sBody = NULL; qUniform = 0.0;};
	~IBMBodyClass() {};
	...
	IBMBodyClass(ObjectsClass *objects, int bodyID, const array<double, dims> &pos, const array<double, dims> &geom,
			double angle, string flex, string nElements, string BC, double rho, double E,  double qUniform = 0.0);

	void setUniformLoad(double q) { qUniform = q; }
    double getUniformLoad() const { return qUniform; }
			// Private members
private:
