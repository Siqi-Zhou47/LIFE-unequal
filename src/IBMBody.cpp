...
IBMBodyClass::IBMBodyClass(ObjectsClass *objects, int bodyID, const array<double, dims> &pos, const array<double, dims> &geom,
		double angle, string flexStr, string nElements, string BC, double rho, double E, double qUniform_in) {
	...
	cerr << "DEBUG IBMBody ctor: resolved flex=" << flex
	<< " (eFlexible=" << eFlexible << " eRigid=" << eRigid << ")"
	<< std::endl << std::flush;
	...
	if (flex == eFlexible){	
		sBody = new FEMBodyClass(this, pos, geom, angle, nElements, BC, rho, E, this->qUniform);
		// sBody->setUniformLoad(this->qUniform);

		// cerr << "DEBUG IBMBody: running staticFEM() for standalone static deformation..." << std::endl;
    	// sBody->staticFEM();  // 直接进行静力学求解
    	// exit(0);
	}
}
