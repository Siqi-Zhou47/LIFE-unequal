...
// ===============================================================
// Guo forcing scheme for Central Moment LBM
// ===============================================================
// inline double GridClass::latticeForce(int id, int v) {

//     // Access global constants
//     const double w[9] = {4.0 / 9.0,
//                          1.0 / 9.0, 1.0 / 9.0,
//                          1.0 / 9.0, 1.0 / 9.0,
//                          1.0 / 36.0, 1.0 / 36.0,
//                          1.0 / 36.0, 1.0 / 36.0};
...
//     // Local macroscopic fields
//     double rho_local = rho[id];
//     if (rho_local < 1e-12) rho_local = 1.0;

//     double ux_local = u[id * dims + eX];
//     double uy_local = u[id * dims + eY];

//     // Local total force (body + IBM)
//     double Fx_local = force_xy[id * dims + eX] + force_ibm[id * dims + eX];
//     double Fy_local = force_xy[id * dims + eY] + force_ibm[id * dims + eY];

//     // Guo forcing term
//     double eu = cx[v] * ux_local + cy[v] * uy_local;
//     double uu = ux_local * ux_local + uy_local * uy_local;
//     double eF = (cx[v] - ux_local) * Fx_local + (cy[v] - uy_local) * Fy_local;

//     return (1.0 - 0.5 * omega) * w[v] * ( (3.0 / SQ(c_s)) * eF + (9.0 / QU(c_s)) * eu * eF - (3.0 / SQ(c_s)) * (ux_local * Fx_local + uy_local * Fy_local) );
...
void GridClass::writeVTK() {
	...
	// ============================================================
	// 新增：计算涡量场
	vector<double> vorticity(Nx * Ny, 0.0);
	...
	for (int i = 1; i < Nx - 1; i++) {
		for (int j = 1; j < Ny - 1; j++) {
			...
			// 涡量: ω = ∂v/∂x - ∂u/∂y
			// 使用中心差分
			double dvdx = (u[((i+1)*Ny + j)*dims + eY] - u[((i-1)*Ny + j)*dims + eY]) / (2.0 * Dx);
			double dudy = (u[(i*Ny + (j+1))*dims + eX] - u[(i*Ny + (j-1))*dims + eX]) / (2.0 * Dx);
			
			// 存储涡量（物理单位）
			vorticity[idx] = (dvdx - dudy) * (Dx / Dt);  // 转换为物理单位 [1/s]
		}
	}
	// ============================================================
	...
	// ============================================================
	// 新增：涡量数据数组
	output << string(level, '\t') << "<DataArray type=\"Float64\" Name=\"Vorticity\" format=\"appended\" offset=\"" << 2*(Nx*Ny*sizeof(double) + sizeof(unsigned long long)) + 3*Nx*Ny*sizeof(double) + sizeof(unsigned long long) << "\"/>\n";
	// ============================================================
	...
	for (int j = 0; j < Ny; j++) {
		for (int i = 0; i < Nx; i++) {
			double vort = vorticity[i * Ny + j];
			output.write((char*)&vort, sizeof(double));
		}
	}
	// ============================================================
