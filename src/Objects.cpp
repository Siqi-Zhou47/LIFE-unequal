...
//     double dt = gPtr->Dt;
//     const double blend = 0.02;              // 从小开始
//     const double tau_relax = 50.0 * dt;    // 松弛时间
...
//         // 流体速度
//         double u_fluid[2] = {0.0, 0.0};
//         if (iNode[i].interpRho > 1e-12) {
//             for (int d = 0; d < dims; d++)
//                 u_fluid[d] = iNode[i].interpMom[d] / iNode[i].interpRho;
//         }
        
//         // 松弛的direct forcing
//         double f_direct[2];
//         for (int d = 0; d < dims; d++) {
//             f_direct[d] = iNode[i].interpRho * (iNode[i].vel[d] - u_fluid[d]) / tau_relax;
//         }
        
//         // 限幅（可选）
//         double ds = iNode[i].ds;
//         double cap = 2.0 * iNode[i].interpRho * 1.0 * 1.0 * ds;
//         for (int d = 0; d < dims; d++) {
//             if (f_direct[d] > cap) f_direct[d] = cap;
//             if (f_direct[d] < -cap) f_direct[d] = -cap;
//         }
        
//         // Blend
//         for (int d = 0; d < dims; d++) {
//             iNode[i].force[d] = (1.0 - blend) * iNode[i].force[d] + blend * f_direct[d];
//         }
//     }
// }
...
//     // *** 调整这些参数 ***
//     const double blend = 0.08;           // 从 0.1 开始（比你的 0.02 更激进）
//     const double tau_relax = 60.0 * dt; // 松弛时间尺度 小 小阻尼
//     const double cap_factor = 3.0;      // 限幅因子
...
//         // 1. 隐式IBM（原始方法）
//         iNode[i].interpolate();
//         iNode[i].forceCalc();  // 计算并存储到 iNode[i].force
...
//         // 4. 限幅（修正版）
//         double ds_physical = iNode[i].ds * gPtr->Dx;  // 转换为物理单位
//         double U_ref = 1.25;  // 入口速度
//         double cap = cap_factor * iNode[i].interpRho * U_ref * U_ref * ds_physical;
...
#pragma omp single
			{
				if (subIt == 1) {
					...
					// Aitken
					// double relax_aitken = -relax * subNum / subDen;
					
					// 限制变化速度
					// double relax_change = relax_aitken - relax;
					// double max_change = 0.3;
					// if (fabs(relax_change) > max_change) {
					// 	relax_change = Utils::sgn(relax_change) * max_change;
					// }
					// relax = relax + relax_change;
					//  // *** 添加范围限制 ***
                    // double relax_min = 0.5;
                    // double relax_max = 0.95;
                    // if (relax < relax_min) relax = relax_min;
...
#pragma omp for schedule(guided)
	   ...
	   // *** 在并行区域外记录日志 ***
    if (!logInitialized) {
        relaxLog.open("Results/RelaxHistory.out");
        relaxLog << "Timestep\tSubIt\tRelax\tSubRes\tSubNum\tSubDen\n";
        logInitialized = true;
    }
    
    if (subIt > 0) {  // 只记录实际的子迭代
        relaxLog << gPtr->t << "\t" << subIt << "\t" 
                 << relax << "\t" << subRes << "\t"
                 << subNum << "\t" << subDen << endl;
    }
	// Compute epsilon
...
// ===========================================================
//  Enhanced recomputeObjectVals()
//  - Aitken dynamic relaxation with adaptive limits
//  - Dual-time coupling support (compatible with IBM dual-scale)
//  - Stable up to blend = 0.1~0.12
// ===========================================================
// void ObjectsClass::recomputeObjectVals() {

//     #pragma omp parallel
//     {
//         // --- Predictor step (first subiteration only)
//         if (subIt == 0) {
//             #pragma omp for schedule(guided)
//             for (size_t ib = 0; ib < iBody.size(); ib++) {
//                 if (iBody[ib].flex == eFlexible) {
...
//             #pragma omp single
//             {
//                 // --- Adaptive Aitken relaxation -----------------
//                 if (subIt == 1) {
//                     // 初始化松弛因子
//                     relax = Utils::sgn(relax) * std::min(std::fabs(relax), relaxMax);
//                 } 
//                 else {
//                     // Aitken 动态松弛更新
//                     double relax_aitken = -relax * subNum / subDen;
...
//                     // 限制更新变化（防止数值震荡）
//                     double relax_change = relax_aitken - relax;
//                     const double max_change = 0.4;
//                     if (std::fabs(relax_change) > max_change)
//                         relax_change = Utils::sgn(relax_change) * max_change;
...
//                     double relax_max = 0.95;
//                     if (subRes > 1e-5) {
//                         relax_min = 0.3;  // 若残差大，适当提高松弛
//                         relax_max = 0.98;
//                     }

//                     if (std::fabs(relax) < relax_min)
//                         relax = Utils::sgn(relax) * relax_min;
//                     if (std::fabs(relax) > relax_max)
//                         relax = Utils::sgn(relax) * relax_max;
//                 }
//             }

//             // --- Apply Aitken-relaxed update to FEM -------------
//             #pragma omp for schedule(guided)
//             for (size_t ib = 0; ib < iBody.size(); ib++) {
//                 if (iBody[ib].flex == eFlexible) {
//                     auto *sb = iBody[ib].sBody;

//                     // U^n+1 = U^n + relax * (U^* - U^n)
//                     sb->U = sb->U_km1 + relax * (sb->U - sb->U_km1);

//                     // Update FEM internal state
//                     sb->updateFEMValues();
//                     sb->finishNewmark();
//                     sb->updateIBMValues();

//                     // Save last converged state
//                     sb->U_km1.swap(sb->U);
//                 }
//             }
//         }

//         // --- Recompute IBM support and epsilon -----------------
//         #pragma omp for schedule(guided)
//         for (size_t i = 0; i < iNode.size(); i++) {
//             if (iNode[i].iPtr->flex == eFlexible) {
//                 iNode[i].findSupport();
...
//     // --- Recompute epsilon (direct forcing sensitivity) -------
//     computeEpsilon();

//     // --- Optional: damp oscillations at startup ---------------
//     if (gPtr->t < 1000) {
//         relax *= 0.9; // startup damping
//     }
// }
	...
	while(file) {
		...
		// ** FILAMENT ** //
    		...
    		if (!restOfLine.empty()) {
        		...
        		if (!(iss >> qUniform)) {
            		qUniform = 0.0; // 如果没读到，就保持 0
       			}
    		}
			// 选择长度变化模式（改这里切换模式！）
			string lengthPattern = "SINUSOIDAL";  // ⬅️ 改这里选择模式
			// 可选值: "SINUSOIDAL", "ALTERNATING", "BLOCK", "GRADED", "UNIFORM"
			
			// 保存基础长度
			double baseLength = geom[eX];
			...
			// === 模式 2: ALTERNATING (交替) ===
			double altLengthLong = 2.0;          // 长filament
			double altLengthShort = 0.8;         // 短filament
			
			// === 模式 3: BLOCK (分块) ===
			int blockSize = 8;                   // 每块filament数量
			double blockLengths[] = {0.8, 1.0, 1.2, 1.4};  // 各块长度值
			int numBlocks = 4;                   // 块的数量
			
			// === 日志文件（可选，用于记录每个filament的长度）===
			static ofstream lengthLog;
			static bool logInitialized = false;
			
			if (!logInitialized && number > 1) {
				lengthLog.open("Results/FilamentLengths.out");
				lengthLog << "# Filament Length Distribution\n";
				lengthLog << "# Pattern: " << lengthPattern << "\n";
				lengthLog << "# BaseLength: " << baseLength << "\n"; 
				lengthLog << "# BodyID\tIndex\tPosX\tPosY\tLength\tNumNodes\n";
				logInitialized = true;
			}
			// Loop through and build
			for (int i = 0; i < number; i++) {
				...
				// Get position vector
				array<double, dims> pos = {start[eX] + i * space[eX], start[eY] + i * space[eY]};
					// 根据选择的模式计算当前 filament 的长度
				if (lengthPattern == "SINUSOIDAL") {
					// 正弦波: L = L_base + A * sin(2π * i / λ)
					double phase = 2.0 * M_PI * i / sinWavelength;
					currentLength = baseLength + sinAmplitude * sin(phase);
					
				} else if (lengthPattern == "ALTERNATING") {
					// 交替: 偶数索引用长值，奇数索引用短值
					currentLength = (i % 2 == 0) ? altLengthLong : altLengthShort;
					
				} else if (lengthPattern == "BLOCK") {
					// 分块: 每 blockSize 个用同一长度，循环使用 blockLengths 数组
					int blockIndex = (i / blockSize) % numBlocks;
					currentLength = blockLengths[blockIndex];
					
				} else {
					// UNIFORM 或其他: 使用基础长度（原始行为）
					currentLength = baseLength;
				}
				
				// ⚠️ 关键步骤：更新 geom[eX] 为新计算的长度
				geom[eX] = currentLength;
				
				// 计算 IBM 节点数（用于日志记录）
				int numNodes = static_cast<int>(floor(currentLength / gPtr->Dx)) + 1;
				
				// 写入日志文件
				if (logInitialized && lengthLog.is_open()) {
					lengthLog << ID + i << "\t" << i << "\t" 
							<< pos[eX] << "\t" << pos[eY] << "\t" 
							<< currentLength << "\t" << numNodes << "\n";
				}
				
				// Call constructor to build it
				iBody.emplace_back(this, ID, pos, geom, angle, flex, nElements, BC, rho, E, qUniform);

				// Set uniform load if provided
        		iBody.back().setUniformLoad(qUniform);
				...
				if (number > 1) {
					...
					if (lengthPattern == "SINUSOIDAL") {
						cout << "\n    Amplitude: ±" << sinAmplitude 
							<< ", Wavelength: " << sinWavelength;
						cout << "\n    Length range: [" 
							<< (baseLength - sinAmplitude) << ", " 
							<< (baseLength + sinAmplitude) << "]";
							
					} else if (lengthPattern == "ALTERNATING") {
						cout << "\n    Long: " << altLengthLong 
							<< ", Short: " << altLengthShort;
						cout << "\n    Difference: " << (altLengthLong - altLengthShort);
...
void ObjectsClass::writeForceDistribution() {
    
    if (!hasIBM) return;
    
    string fname = "Results/ForceDistribution.out";
    bool existing = boost::filesystem::exists(fname);
    ...
    if (!output.is_open())
        ERROR("Error opening force distribution file...exiting");
    
    // === 表头（只写一次）===
    if (!existing) {
        output << "# Force distribution for each body\n";
        output << "# Use for: Length vs Cy analysis\n";
        output << "# Columns: Timestep Time(s)";
        
        // 每个body输出 Cy
        for (size_t ib = 0; ib < iBody.size(); ib++) {
            output << " Cy_" << iBody[ib].ID;
        }
        output << "\n";
    }
    
    // === 输出当前时间步 ===
    output << gPtr->t << "\t" << gPtr->Dt * gPtr->t;
    ...
    for (size_t ib = 0; ib < iBody.size(); ib++) {
        ...
        for (size_t n = 0; n < iBody[ib].node.size(); n++) {
            forceY -= iBody[ib].node[n]->force[eY] * 
                     iBody[ib].node[n]->epsilon * 
                     iBody[ib].node[n]->ds * forceScale;
        }
...
// ------------------------------------------------------------
// 函数2: 时间平均统计量
// 目的: 量化对比、Length vs Response 图表
// 调用: 模拟结束时调用一次（在主循环最后）
// ------------------------------------------------------------

void ObjectsClass::writeSpatialStatistics() {
    
    if (!hasIBM) return;
    
    cout << "\n  *** Computing spatial statistics from time series data ***\n";
    
    // === 读取现有输出文件并计算统计量 ===
    
    // 1. 读取 TipPositions.out
    ifstream tipPosFile("Results/TipPositions.out");
    if (!tipPosFile.is_open()) {
        WARN("TipPositions.out not found, skipping statistics");
        return;
    }
    
    // 跳过表头
    string header;
    getline(tipPosFile, header);
    
    // 确定有多少个柔性body
    int nFlexBodies = 0;
    for (size_t ib = 0; ib < iBody.size(); ib++) {
        if (iBody[ib].flex == eFlexible) nFlexBodies++;
    }
    
    // 存储数据
    vector<vector<double>> tipY_data;  // [body][timestep]
    
    // 读取数据
    string line;
    while (getline(tipPosFile, line)) {
        ...
        vector<double> tipY_timestep;
        for (int ib = 0; ib < nFlexBodies; ib++) {
            double tipX, tipY;
            iss >> tipX >> tipY;
            tipY_timestep.push_back(tipY);
        }
        
        tipY_data.push_back(tipY_timestep);
    }
    tipPosFile.close();
    
    int nSamples = tipY_data.size();
    
    // 2. 读取 ForceDistribution.out
    ifstream forceFile("Results/ForceDistribution.out");
    vector<vector<double>> Cy_data;  // [body][timestep]
    
    if (forceFile.is_open()) {
        // 跳过注释行
        while (getline(forceFile, line) && line[0] == '#');
        ...
        while (getline(forceFile, line)) {
            ...
            vector<double> Cy_timestep;
            double cy;
            while (iss >> cy) {
                Cy_timestep.push_back(cy);
            }
            
            Cy_data.push_back(Cy_timestep);
        }
        forceFile.close();
    }
    ...
    string fname = "Results/SpatialStatistics.out";
    ofstream output;
    output.open(fname.c_str());
    output.precision(PRECISION);
    
    if (!output.is_open())
        ERROR("Error opening spatial statistics file...exiting");
    
    // 表头
    output << "# Spatial statistics (time-averaged)\n";
    output << "# Computed from " << nSamples << " samples\n";
    output << "# Use for: Length vs Response scatter plots\n";
    output << "#\n";
    output << "# Columns:\n";
    output << "#   BodyID  - Body identifier\n";
    output << "#   PosX    - X position of first node\n";
    output << "#   Length  - Filament length\n";
    output << "#   TipY_mean - Time-averaged tip Y position\n";
    output << "#   TipY_std  - Standard deviation of tip Y\n";
    output << "#   TipY_rms  - RMS of tip Y\n";
    output << "#   A_norm    - Normalized amplitude (TipY_std / Length)\n";
    output << "#   Cy_mean   - Time-averaged lift coefficient\n";
    output << "#   Cy_rms    - RMS of lift coefficient\n";
    output << "#\n";
    output << "BodyID\tPosX\tLength\tTipY_mean\tTipY_std\tTipY_rms\tA_norm\tCy_mean\tCy_rms\n";
    
    // 逐个body计算
    int flexIdx = 0;  // 柔性body的索引
    for (size_t ib = 0; ib < iBody.size(); ib++) {
        
        if (iBody[ib].flex != eFlexible) continue;
        
        // 获取长度
        double length = iBody[ib].sBody->L0;
        
        // 获取基准位置
        double posX = iBody[ib].node[0]->pos[eX];
        
        // 计算 TipY 统计量
        double tipY_sum = 0.0;
        double tipY_sq_sum = 0.0;
        
        for (int i = 0; i < nSamples; i++) {
            double tipY = tipY_data[i][flexIdx];
            tipY_sum += tipY;
            tipY_sq_sum += tipY * tipY;
        }
        
        double tipY_mean = tipY_sum / nSamples;
        double tipY_rms = sqrt(tipY_sq_sum / nSamples);
        double tipY_variance = (tipY_sq_sum / nSamples) - (tipY_mean * tipY_mean);
        double tipY_std = sqrt(max(0.0, tipY_variance));  // 防止负数
        
        // 归一化振幅
        double A_norm = tipY_std / length;
        
        // 计算 Cy 统计量
        double Cy_mean = 0.0;
        double Cy_rms = 0.0;
        
        if (Cy_data.size() > 0 && Cy_data[0].size() > ib) {
            double Cy_sum = 0.0;
            double Cy_sq_sum = 0.0;
            
            int nCySamples = min(nSamples, (int)Cy_data.size());
            for (int i = 0; i < nCySamples; i++) {
                double cy = Cy_data[i][ib];
                Cy_sum += cy;
                Cy_sq_sum += cy * cy;
            }
            
            Cy_mean = Cy_sum / nCySamples;
            Cy_rms = sqrt(Cy_sq_sum / nCySamples);
        }
    ...
    cout << "  *** Statistics written to " << fname << " ***\n";
}
