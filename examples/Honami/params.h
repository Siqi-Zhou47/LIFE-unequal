...
#define INLET_RAMP 150.0			// Inlet velocity ramp-up
...
#define WALL_RIGHT	eConvective //ePressure			// Boundary condition at right wall
#define WALL_BOTTOM	eWall				// Boundary condition at bottom wall
#define WALL_TOP	eFreeSlip		// Boundary condition at top wall
#define PROFILE 	eParabolic //eBoundaryLayer		// Profile shape (uncomment for uniform)

// Inlet conditions
const double uxInlet_p = 1; //0.006;			// Inlet x-velocity (m/s)
...
const double relaxMax = 0.8;//1.0			// Relaxation parameter (initial guess)
const double subTol = 1e-8;//8				// Tolerance for subiterations
...
// Set time step
const double tStep = 0.0005; //0.0005; //0.001;//0.00625 / (4.0 * 4.0);
...
const int nSteps = 1200000;							// Number of timesteps
const int tinfo = 60;								// Frequency to write out info and logs
const int tVTK = 1200;								// Frequency to write out VTK
const int tRestart = 12000;							// Frequency to write out restart files
